...
function init(form,target) {
	total_msg = '';
	any_error = false;
	if (!first) {	//If we've run checkError before, remove the previous errors
...
function checkErrors(form,target,alert_msg) {
		...
		if (any_error) {
			...
			document.getElementById(target).appendChild(errorTable);
                        window.scroll(0,0);
			return false;
		}
...
function validate() {
	...
	var highlight = false;	//Should this field be highlighed if an error is found?
    ...
    case "minage":
      ...
      break;
		...
		case "length":
			...
			break;
                ...
                case "minlength":
                        ...
                        var highlight = true;
                        var default_msg = "Please enter more than " + min_length + " characters. (currently " + field.value.length + " characters)";
		...
		case "time":
			...
			if (field.value != "") {
				if (!time_pattern.test(field.value) && !time_pattern2.test(field.value) && !time_pattern3.test(field.value) && !time_pattern4.test(field.value)) {
					error = true;
				}
			}
			break;
		...
		case "email":
			...
			if (domain != "" && domain != null) {
			  email_pattern = new RegExp("^[^ ]+\@("+domain+")\.[^ ]+$");
  ...
  case 'negateRegex':
	...
	else if (highlight && field.value != "" && field.getAttribute("className") == "error") {
		field.removeAttribute("class");
		field.removeAttribute("className");		// For IE
	}
}
...
/**
 * Temporary function used to carry over debug parameters when clicking links.
 * TODO @skalmakis. Move to google3 once initLinks is submitted
 *
 * @param {string} debug value of debug URL parameter, if it exists
 * @param {string} test value of test URL parameter, if it exists
 **/
function setDebugURLs(debug, test) {
  var debugParams = '';
  //determine the present debug values
  if (debug) {
    debugParams = "debug=" + debug;
  }
  if (test) {
    if (debugParams.length > 0) {
      debugParams = debugParams + "&test=" + test;
    } else {
      debugParams = "test=" + test;
    }
  }
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      if (link.href.indexOf('javascript') == -1) {
        if (link.href.indexOf('?') >= 0) {
          link.href = link.href + "&" + debugParams;
        } else {
          link.href = link.href + "?" + debugParams;
        }
      }
